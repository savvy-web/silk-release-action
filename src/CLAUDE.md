...
- **Schema** -- `schema/inputs.ts` and `schema/outputs.ts` are the single decode/declare points (see below); `release-output.ts`, `projections.ts`, `silk-release-config.ts`. Both JSON Schema artifacts live under `schemas/`, generated by `pnpm schema:build` from `lib/scripts/schemastore.config.ts`: the input schema unversioned at `schemas/silk-release-action.input.schema.json`, the output schema **versioned** at `schemas/<version>/silk-release-action-<version>.json` (currently `5.2.0`; `SCHEMA_URL` must equal the config-derived `$id`). **Numeric fields: `Schema.Int` unless the field genuinely holds a fraction, `Schema.Finite` as the floor, never bare `Schema.Number`** — `Schema.Number` is a lossless JSON codec that round-trips `Infinity`/`NaN` as string sentinels, so it lowers to an `anyOf` with those literals instead of `{"type":"integer"}`. All 31 numeric fields in `release-output.ts` are `Schema.Int`
