...
- **Schema** -- `schema/inputs.ts` and `schema/outputs.ts` are the single decode/declare points (see below); `release-output.ts`, `projections.ts`, `silk-release-config.ts`. Input JSON Schema artifact at repo root; the output schema is **versioned** at `schemas/<SCHEMA_SEMVER>/silk-release-action-<SCHEMA_SEMVER>.json` (currently `5.0.0`)
