...
- **Release module** -- `release/`: `publish.ts` (`detectReleases` → `planWorkspaces` → `runBuildAndSbom` → `runPublishTargets` → `runReleases`), `releases.ts` (group-keyed asset names), `validation.ts`, `validation-checks.ts` (`deriveValidationChecks`, pure), `report.ts`, `publishability.ts`, `changeset-config.ts`, `meta-archive.ts`, `attest-helpers.ts`, `resolve-targets.ts` (`resolvePublishTargetSpecs` — the one detect + JSR-filter + private-build-filter shared by `planWorkspaces`, `runBuildAndSbom` and `runPublishTargets` so they cannot disagree; fails with `PublishTargetBindingError` when detection misses the `dist/prod/targets.json` binding), `types.ts`, `errors.ts`, `layers.ts` (`ReleaseLive` — workspace discovery, `ChangesetConfig`, the adaptive `PublishabilityDetector`, and the silk-effects `ReleasePlanner`/`ConfigInspector`)
- **Schema** -- `schema/inputs.ts` and `schema/outputs.ts` are the single decode/declare points (see below); `release-output.ts`, `projections.ts`, `silk-release-config.ts`. Input JSON Schema artifact at repo root; the output schema is **versioned** at `schemas/<SCHEMA_SEMVER>/silk-release-action-<SCHEMA_SEMVER>.json` (currently `5.0.0`). **Numeric fields: `Schema.Int` unless the field genuinely holds a fraction, `Schema.Finite` as the floor, never bare `Schema.Number`** — `Schema.Number` is a lossless JSON codec that round-trips `Infinity`/`NaN` as string sentinels, so it lowers to an `anyOf` with those literals instead of `{"type":"integer"}`. All 31 numeric fields in `release-output.ts` are `Schema.Int`
...
- **Utilities** -- `utils/*.ts`, one focused purpose each, notably `native-version.ts`, `write-sections.ts` (the fold shared by both phases), `sort-releases-topologically.ts`, `group-id.ts`, `detect-workflow-phase.ts`, `release-kind.ts` (`github-only` vs `github-with-packages`, the classification every phase's output now shares)
...
- **A package `version` is optional.** `@effected/workspaces` widened `WorkspacePackage.version` to an optional key and `PublishablePackage.version` to `string | undefined` — publishability is decided by `publishConfig`, and a version-less private package or monorepo root is an ordinary pnpm shape. Answer the absence, never substitute `"0.0.0"`: render labels through `labelPackage` in `utils/release-summary-helpers.ts` (bare name when absent, or a release PR title reads `name@undefined`), skip a version-less manifest in `release/validation.ts` detection rather than releasing it version-less, and read `currentVersion` off `ReleasedPackage` rather than re-reading `pkg.version`
...
Two kit-owned idioms, do not re-localise them: parse closing-keyword issue references with `collectReferenceLists` from **`@effected/github-references`** (issue #261 — the grammar moved out of `@effected/github`, whose compat re-export is droppable; the local `CLOSE_KEYWORD_PATTERN` regex was deleted, do not reintroduce one, and filter on `closing` since `Refs` associates without closing), and make idempotent bot comments with `GitHubIssue.commentOnce` + `CommentMarker` (create-or-skip, never edit; `close-linked-issues` closes *before* commenting as belt-and-suspenders, since the marker lookup is not atomic).

An **already-closed** linked issue is no longer skipped outright (issue #259): `utils/issue-close-attribution.ts` asks GitHub which pull requests closed it, and only a closure attributable to *this* release PR falls through to the comment — recovering a close whose comment never landed without posting a false claim on an issue the merge did not close. Attribution that cannot be established degrades to the skip, never to a claim. The query is a consumer-owned `GraphQLDocument`, which is the kit's stated extension point, not a raw-client escape.

**Managed sections use the kit's region engine** (issue #258, landed). `utils/managed-sections.ts` no longer scans or splices regions: `ManagedDocument` owns the grammar, the stamp lives in region **metadata** (`state`/`sha`/`runId`/`at`, with `at`/`runId` spelled bare so a later `CheckDocument` swap inherits its drop rule), each section's banner is its own `<key>-banner` region, and rule 5 delegates to `CheckDocumentStamp.isAtLeastAsRecent`. The old `silk-release:section:<key>:start` wire format is **converted, not stripped**, by `migrateLegacySections` on every read path — stripping would delete the sections the writing run does not own. A legacy region whose stamp will not decode is left in place as prose.
