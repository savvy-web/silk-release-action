...
- **`layers/`** -- `app.ts` builds `MainLive` from kit services; `ActionEnvironment`, `ActionLogger`, `ActionOutputs`, `ActionState` and `NodeServices` come from `ActionRuntime` via `Action.run` and must be *required*, never rebuilt. `identity-token.ts` wires OIDC
