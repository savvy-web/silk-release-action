...
- **Release module** -- `release/` (phase orchestration): `publish.ts` (Phase-3 publish chain, token-auth fallback), `releases.ts` (GitHub releases, group-keyed asset names), `validation.ts` (Phase-2 validation, check-summary byte-capping), `report.ts` (comment/summary renderers), `publishability.ts` (silkDetect logic), `changeset-config.ts` (ChangesetConfig service), `meta-archive.ts` (`tarMetaFolder` — packs the bundler `meta/` folder into the group `meta.tgz` API-docs bundle), `attest-helpers.ts`, `resolve-targets.ts` (resolves per-package targets from each `dist/prod/targets.json` binding), `types.ts`, `layers.ts` (includes `NativeVersioningLive` — silk-effects v3 `ReleasePlanner`/`ConfigInspector` layers for Phase-1 versioning), `errors.ts`; co-located `*.test.ts` for each module
- **Changelog workers** -- `changelog/silk.ts` and `changelog/default.ts` are bundled to `dist/changelog-silk.js` / `dist/changelog-default.js` via `action.config.ts` `workers` entries; `build.nativeDynamicImports` keeps the runtime dynamic imports in `@changesets/apply-release-plan` and `workspaces-effect` native
