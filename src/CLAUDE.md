...
- **Schema** -- `schema/inputs.ts` and `schema/outputs.ts` are the single decode/declare points (see below); `release-output.ts`, `projections.ts`, `silk-release-config.ts`. All three JSON Schema artifacts are **versioned** under `schemas/<version>/` (`output.json`, `input.json`, `sbom-template.json`; currently `6.0`), generated by `pnpm schema:build` from `lib/scripts/schemastore.config.ts`. `SCHEMA_URL`/`INPUT_SCHEMA_URL`/`SBOM_TEMPLATE_SCHEMA_URL` are the `$id` getters of the three `HostedSchema` identities `silk-release-config.ts` builds (`OutputSchemaIdentity`/`InputSchemaIdentity`/`SbomTemplateSchemaIdentity`, `HostedSchema.github({ repo, path: "schemas", name, versions: OUTPUT_SCHEMA_VERSIONS, current: OUTPUT_SCHEMA_VERSION })`); the config receives the same values as `hosted`, so there is no second derivation to keep in step. **Numeric fields: `Schema.Int` unless the field genuinely holds a fraction, `Schema.Finite` as the floor, never bare `Schema.Number`** — `Schema.Number` is a lossless JSON codec that round-trips `Infinity`/`NaN` as string sentinels, so it lowers to an `anyOf` with those literals instead of `{"type":"integer"}`. All 31 numeric fields in `release-output.ts` are `Schema.Int`
