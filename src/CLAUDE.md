...
- **Utility modules** -- `utils/*.ts` for individual operations, including `group-id.ts` (`getGroupId`, `insertGroupToken` — derive the byte-group id from a `dist/prod/<group>/pkg` directory for group-keyed asset names), `create-validation-check.ts` (check-summary byte-capping), `normalize-package-manager.ts` (narrow the `packageManager` input so Phase-2 dry-runs and Phase-3 publish dispatch through the same npm executor) and `registry-label.ts` (`registryShortLabel`/`registryHost` for the publish and validation log-tree rows)
