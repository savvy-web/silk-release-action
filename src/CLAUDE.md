...
- **Schema** -- `schema/inputs.ts` and `schema/outputs.ts` are the single decode/declare points (see below); `release-output.ts`, `projections.ts`, `silk-release-config.ts`. Both JSON Schema artifacts are **versioned** under `schemas/<version>/` (`silk-release-action.output-<version>.json`, `silk-release-action.input-<version>.json`; currently `5.2`), generated by `pnpm schema:build` from `lib/scripts/schemastore.config.ts`. `SCHEMA_URL`/`INPUT_SCHEMA_URL` are derived from `OUTPUT_SCHEMA_URL` + `OUTPUT_SCHEMA_VERSION` (`silk-release-config.ts`) and must equal the config-derived `$id`s. **Numeric fields: `Schema.Int` unless the field genuinely holds a fraction, `Schema.Finite` as the floor, never bare `Schema.Number`** — `Schema.Number` is a lossless JSON codec that round-trips `Infinity`/`NaN` as string sentinels, so it lowers to an `anyOf` with those literals instead of `{"type":"integer"}`. All 31 numeric fields in `release-output.ts` are `Schema.Int`
