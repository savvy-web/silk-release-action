...
| `pnpm schema:build` | Regenerate the JSON Schema artifacts under `schemas/` (the versioned output schema and `silk-release-action.input.schema.json`) |
...
```bash
pnpm schema:check
# unchanged .../schemas/5.2.0/silk-release-action-5.2.0.json [policy semantic]
# unchanged .../schemas/silk-release-action.input.schema.json [policy semantic]

pnpm schema:build
# written (contract) ... — rewrites in place while the label is unpublished
```

The output schema's label is unpublished (`published: false`) today, so a contract change regenerates `schemas/5.2.0/` in place. Once a label ships, flip `published` and respond to drift by appending a new label to `versions` and moving `SCHEMA_URL` in `src/schema/release-output.ts` with it — the derived `$id` and `SCHEMA_URL` must be identical, and `__test__/schemastore-config.test.ts` pins them together. `--force` (sugar for `--drift=allow`) writes through drift at a published label; reach for it only to repair a document whose committed text no longer parses. Runbook: `okf/runbooks/bump-output-schema-version.md`.
