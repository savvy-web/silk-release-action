...
The publish/SBOM engine underneath is **`src/release/validation.ts`** (`runValidation`), a pure Effect program. It enumerates workspace packages, diffs versions against the target branch to discover which packages are being released (`detectReleasedPackages` drops changeset-ignored names entirely via `ChangesetConfig.isIgnored`, so they never appear, not even as version-only rows, and skips a package whose manifest declares no `version` at all — there is nothing to release *under*, and classing it brand-new would release it version-less; see the optional-`version` contract under [Shared Infrastructure](#shared-infrastructure)), orders the released set dependency-first through `sortReleasesTopologically` (matching Phase-3 order; a cyclic graph falls back to discovery order rather than aborting), resolves publish targets via `resolvePublishableTargets`, groups targets by build directory, runs `PackagePublish.dryRun` per build directory, generates one SBOM per build directory, applies `sbom-config` metadata, and assembles a `ValidationReport`. The report is build-centric: `ValidationPackageResult` carries builds, sizes, SBOMs, and registry targets. Downstream of detection the version comes from `ReleasedPackage.currentVersion` — the value already narrowed at detection time — not from re-reading the now-optional `pkg.version`. `strict-warnings` mode escalates warning-severity findings to `failure` for auto-merge gating.
...
- **`src/utils/release-summary-helpers.ts`** — release-title and package-listing helpers built on the single `PublishabilityDetector`. `listPublishablePackages(root)` is an Effect over `WorkspaceDiscovery` + `PublishabilityDetector`; `getReleasingPackages`, `resolveReleasePrTitle` and `formatReleasePackageList` are pure. A package's `version` is **optional** — `@effected/workspaces` types `WorkspacePackage.version` as an `optionalKey` and `PublishablePackage.version` as `string | undefined`, because publishability is decided by `publishConfig` and a version-less private package or monorepo root is an ordinary pnpm shape. The upstream contract is that a consumer answers the absence rather than inventing a `"0.0.0"` placeholder, so `resolveReleasePrTitle` and `formatReleasePackageList` take `version: string | undefined` and render through a private `labelPackage(name, version)` that emits `name@version` or the bare `name`. Interpolating the absent value instead would put a literal `name@undefined` into a release PR title and a commit body.
...
- **`src/schema/release-output.ts`** — `ReleaseOutput` as a `Schema.Union` of three phase structs discriminated by the `phase` literal: `BranchManagementOutput`, `ValidationOutput`, `PublishOutput`. **Schema v2** (`SCHEMA_VERSION = "2"`, in-band as every payload's `schemaVersion` field): every phase now carries an orthogonal `success` (boolean gate) + `outcome` (per-phase taxonomy) pair, plus a derived `summary` (one sentence, computed from the structured fields beside it — never authored independently, so the prose cannot drift from the data), a `failure` record (`stage` + `reason`, null on success), and `totals`. The old four-flag set (`status`/`noop`/`succeeded`/`hasFailures`) and `deriveStatus` are gone; so are `ReleaseFlags` and `StatusLiteral`.
  ...
  Every numeric field in this module is `Schema.Int`. All of them are counts, byte sizes, a PR number and a GitHub release id — integers — so `Schema.Int` is both the honest type and the one that lowers to a plain `{ "type": "integer" }` carrying its `title`/`description`. The alternative is worse than it looks: `Schema.Number` is a *lossless* JSON codec, round-tripping `Infinity`/`-Infinity`/`NaN` through string sentinels, so it lowers to `anyOf: [number, "NaN"|"Infinity"|"-Infinity"]` and that lowering drops the annotation when it hoists the union into `$defs`. The module used to mix `Schema.Number` and `Schema.Finite` for no reason anyone could name; a new numeric field here should be `Schema.Int` unless it genuinely holds a fraction, in which case `Schema.Finite` is the floor — never bare `Schema.Number`.
...
**Generation runs through `@effected/schemastore`'s `SchemaPipeline`**, which replaced the hand-rolled Draft-07 lowering plus the `biome` shell-out and the `ajv` dependency (`ajv` is gone from `package.json`). The pipeline handles the Draft 2020-12 → Draft-07 lowering, strict validation and drift-stable formatting; `__test__/generate-schema.test.ts` is the drift guard over both committed documents. **The contract gate is the pipeline's own**, not this repo's: `SchemaPipeline.run` and `.check` take a `contractChanges` policy, and the default `block-versioned` refuses to rewrite an already-published version in place when the change is a contract change — a removed or renamed field, a changed type — failing with a `SchemaContractChangeError` that names every affected document and the `nextVersion` it suggests, rather than silently rewriting a version's file out from under consumers pinned to its URL. `lib/scripts/generate-schema.ts` used to duplicate that check with a hand-rolled `SchemaPipeline.check` preflight filtering `change === "contract"`; that preflight is gone. The response to a genuine contract break is still bumping `SCHEMA_SEMVER` (and `SCHEMA_URL`) to a new label, which writes a new file and leaves the published one alone. Two flags cover the rest: `--check` (alias `--dry-run`) reports what a run would do — reading `contractBlocked` and `wouldWrite` off the check result — and writes nothing; `--allow-contract-change` (alias `--force`) sets the policy to `allow` and writes through the gate at the same label, after logging a loud warning, for a label that is not yet published or a published file that is unparseable.
