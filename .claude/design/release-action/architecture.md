...
The action consumes a deliberately narrow slice of its first-party libraries: `ChangesetConfig`, the silk publishability rules (`SilkPublishability` / `PublishabilityDetectorAdaptiveLive`), `WorkspaceDiscovery`, the `DependencyGraph` topological sorter, the sync discovery helpers (all from `@effected/workspaces` — the Effect-v4 successor to `workspaces-effect`; `findWorkspaceRootSync` / `getWorkspacePackagesSync` now take `nodeSyncOps` from `@effected/workspaces/node-sync`) and — since the native-versioning rework — the `@savvy-web/silk-effects` 4.x changesets engine (`Changesets.ReleasePlanner` / `Changesets.ConfigInspector` plus the bundled changelog generators). Phase 1 versions in-process through that engine, so consumer `ci:version` scripts are never invoked and Phase 1 runs zero-install (the shared workflow passes `install-deps: false`). See [Native versioning (zero-install)](#native-versioning-zero-install).
...
- **`src/pre.ts`** — Pre-action setup. Provisions a GitHub App installation token via `GitHubToken.provision()` using the `app-client-id` and `app-private-key` inputs. `provision` persists the `InstallationToken` envelope (token, expiry, installation ID, and resolved App identity) to `ActionState` under an internal key; under 2.0 the `InstallationToken.token` field decodes to `Redacted<string>`. Also records the optional `github-token` input to `GithubPackagesTokenState` in `src/state.ts`. Failures here abort the workflow. The `PreLive` layer composes `GitHubAppLive` over `OctokitAuthAppLive` plus `FetchHttpClient.layer`, with the single `NodeServices.layer` backing `ActionStateLive`; because `GitHubAppLive` requires `HttpClient.HttpClient`, both `pre.ts` and `post.ts` provide `FetchHttpClient.layer` (now imported from `effect/unstable/http`). Under Effect v4 the separate `NodeContext.layer` / `NodeFileSystem.layer` / `NodePath.layer` platform layers collapse into one `NodeServices.layer` (from `@effect/platform-node`), which provides `FileSystem`, `Path`, and the rest of the platform services.
...
Phase 1 versions in-process — the action no longer shells out to the consumer's `ci:version` script and the `version-command` input is removed from `action.yml`. Both branch-management flows call `runNativeVersion(cwd)` (`src/utils/native-version.ts`), which drives the bundled silk-effects v4 `Changesets.ReleasePlanner.apply` — the same engine `savvy changeset version` runs — after validating `.changeset/config.json` via `Changesets.ConfigInspector` (an absent config proceeds, matching the savvy CLI's gate). Since main.ts no longer needs to pick a version command, both call sites are argument-free (no `packageManager` parameter).
...
- **Format-preserving versionFiles writes** — the bundled silk-effects engine applies changesets `versionFiles` bumps as in-place jsonc edits rather than a `JSON.parse`/`JSON.stringify` round trip, so a version bump produces a one-line diff and inline arrays, indentation and JSONC comments survive byte-for-byte (a missing wildcard-free JSONPath leaf is inserted with detected indentation). Formatting integrity therefore no longer depends on the post-version Biome pass — which matters because that pass silently skips in zero-install runs when the consumer's biome config extends a `node_modules` package.
- **Post-version formatting** — `formatWorkspaceWithBiome` (`src/utils/format-workspace.ts`) replaces the `&& biome format --write .` tail of the removed consumer script; see [the format policy](integration.md#native-versioning-and-zero-install-phase-1) for the warn-vs-fail rules. Since versionFiles writes became format-preserving this pass is cosmetic cleanup, not a repair step.
- **Layer** — `NativeVersioningLive` in `src/release/layers.ts` composes `ReleasePlannerLive` over `ConfigInspectorLive`, provided by `ChangesetConfigReaderLive` and the git-free `Workspaces.layer()` composite (bound to the `WorkspacesLive` const), and is merged into `ReleaseLive`. The platform `FileSystem`/`Path` it depends on come from `NodeServices.layer`, provided at the `main.ts` boundary.
...
Publish and release operate on the `@savvy-web/bundler` prod layout. Each package's `publishConfig.targets` is a Record map resolved through a `dist/prod/targets.json` binding (the resolution lives in the `PublishabilityDetector` from `@savvy-web/silk-effects` / `@effected/workspaces`, not in this repo), and the build emits one `dist/prod/<group>/pkg` directory per byte-variant group with a sibling `dist/prod/<group>/meta` folder. Registries whose bytes are identical share one group — `npm: true` + `github: true` collapse into a single `dist/prod/<group>/pkg` tarball deployed to both. The orchestrators still pack once per build directory; the per-group layout means a build directory now corresponds to a byte-group rather than a single per-registry publish dir. `src/utils/group-id.ts` derives the group id from a build directory (`getGroupId("dist/prod/npm/pkg") === "npm"`) for asset naming and step labels.
...
main.ts
  ...
  |       +-- sort-releases-topologically.ts (sortReleasesTopologically → DependencyGraph.sortSubset)
  ...
  |     release/layers.ts
  |       WorkspacesLive = Workspaces.layer()  (@effected/workspaces git-free composite:
  |                        WorkspaceDiscovery + PublishabilityDetector + DependencyGraph)
  ...
  |       (platform FileSystem/Path via NodeServices.layer at the main.ts boundary)
  |
  +-- Phase 3a:
...
- **`src/release/layers.ts`** — Release-orchestration layer provisioning. `ReleaseLive` composes the git-free `Workspaces.layer()` composite (a parameterized factory bound once to the `WorkspacesLive` const, providing `WorkspaceDiscovery` + `PublishabilityDetector` + `DependencyGraph` from `@effected/workspaces`), `ChangesetConfigLive`, the adaptive publishability override, and `NativeVersioningLive`. Topological ordering no longer flows through a `TopologicalSorter` service — `sort-releases-topologically.ts` builds a `DependencyGraph.make({ packages })` from `WorkspaceDiscovery.listPackages()` and calls `sortSubset`. Under Effect v4 the platform services (`FileSystem`, `Path`, etc.) are provided once at the `main.ts` boundary via the single `NodeServices.layer` (from `@effect/platform-node`), which replaced the separate `NodeContext`/`NodeFileSystem`/`NodePath` layers; `main.ts`'s `MainLive` stack provides it to `ActionStateLive`, `ReleaseLive`, `ActionOutputsLive`, and `ChangesetAnalyzerLive`, so `R` resolves to `never`.
...
Two JSON Schema artifacts at repo root are generated from these schemas: `silk-release-action.input.schema.json` and `silk-release-action.output.schema.json`. Both carry `$id` URLs pointing to `raw.githubusercontent.com/savvy-web/silk-release-action/main/`. Under Effect v4 the generator (`lib/scripts/generate-schema.ts`) was rewritten against effect-core JSON Schema — `Schema.toJsonSchemaDocument` (Draft 2020-12) lowered to Draft-07 via `JsonSchema.toDocumentDraft07` (from `effect`, replacing the dropped `json-schema-effect`), ajv-strict validated, and Biome-formatted for drift-stable writes. The exported `buildActionJsonSchema(schema, $id)` is pure so the drift guard `__test__/generate-schema.test.ts` calls it directly. Both committed documents were regenerated; their root reshaped to a top-level `$ref` into the `$defs` pool (a visible but valid Draft-07 SchemaStore/editor-schema change — not the `action.yml` contract).
