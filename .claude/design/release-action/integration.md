...
`pickToken(registry, npmToken, ghPkgsToken, customTokens)` in the same module resolves each target registry's credential from **one** `classifyRegistry` call, switched exhaustively — rather than two independent booleans asked in sequence, which can disagree:
...
- `"jsr"` (`jsr.io`) — publishes over OIDC and has no token of its own. It shares the custom-registry resolution below only because the predecessor's two booleans both answered false for it and it fell through; preserved deliberately rather than changed inside a migration
- `"custom"` — the `custom-registries` input first (parsed by `utils/custom-registries.ts` into trailing-slash-normalized URL → `Redacted` token pairs, restored for issue #215), then an env var derived from the URL as the fallback (e.g. `https://registry.savvyweb.dev/` → `REGISTRY_SAVVYWEB_DEV_TOKEN`)

The `custom-registries` tokens stay `Redacted` from the decode in `schema/inputs.ts` down to `runPublishTargets`, which declassifies each once through `Secret.forSigning` — mask-first, so the token is registered with the runner's log filter before any plaintext exists — and hands it to `setupAuth`'s npmrc write. There is **no process-environment bridge**; the plaintext-env `STATE_githubToken` bridge the pre-#90 implementation used was deleted, not moved (see `program.ts`). `customTokens` has no default value at the `pickToken` signature on purpose: the one production caller must thread the parsed input, so severing the wiring is a compile error rather than a silent regression. A malformed input line (`_auth=<base64>` basic auth, a bare URL, a duplicate registry) fails the decode typed in `schema/inputs.ts` instead of being silently ignored.
