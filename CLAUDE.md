...
**Source layout.** `main.ts` is a 19-line entry guard only; composition lives in `src/program.ts` (read inputs → detect phase → five-arm switch) and the layer graph in `src/layers/`. Phase bodies are one module each under `src/steps/`. Also: `src/release/` (publish, releases, validation, reporting, publishability, `resolve-targets.ts`), `src/schema/` (`inputs.ts` and `outputs.ts` are the single decode/declare points, plus the output schema and projections), `src/utils/` (including `release-kind.ts`), `src/types/`, `src/changelog/` (bundled changelog workers). JSON Schema artifacts, both under `schemas/`: the unversioned input schema `schemas/silk-release-action.input.schema.json`, and the **versioned** `schemas/5.2.0/silk-release-action-5.2.0.json` (the output document — kept under its own version label so a newer schema can't silently move underneath a fixed path). Both are generated by `@effected/schemastore-cli` from `lib/scripts/schemastore.config.ts`, which the scripts pass to the CLI explicitly (`src/` is action source only) and which declares the two Effect Schema sources and derives every path and `$id` from `outputDir: "schemas"` + `baseUrl` — there is no generator script and nothing is spelled by hand. `pnpm schema:build` writes; `pnpm schema:check` is the same walk with no writes and fails when a build would write (stale artifact) or refuse (gate failure, or drift at a `published: true` label). The output label is `published: false` today — the schema has never shipped — so a contract change regenerates `5.2.0` in place. Once a label is published, drift is answered by appending a new label to `versions` and moving `SCHEMA_URL` (`src/schema/release-output.ts`) with it; `__test__/schemastore-config.test.ts` pins `SCHEMA_URL`/`INPUT_SCHEMA_URL` to the config-derived `$id`s. `--force` (sugar for `--drift=allow`) rewrites a published document in place — only to repair unparseable text. Runbook: `okf/runbooks/bump-output-schema-version.md`.
