...
**Source layout.** `main.ts` is a 19-line entry guard only; composition lives in `src/program.ts` (read inputs → detect phase → five-arm switch) and the layer graph in `src/layers/`. Phase bodies are one module each under `src/steps/`. Also: `src/release/` (publish, releases, validation, reporting, publishability, `resolve-targets.ts`), `src/schema/` (`inputs.ts` and `outputs.ts` are the single decode/declare points, plus the output schema and projections), `src/utils/` (including `release-kind.ts`), `src/types/`, `src/changelog/` (bundled changelog workers). JSON Schema artifacts, both **versioned** under `schemas/<version>/`: `silk-release-action.output-<version>.json` (the `result` document) and `silk-release-action.input-<version>.json` (the `sbom-config`/`.github/silk-release.json` shape), `schemas/5.2/` today — kept under a version label so a newer schema can't silently move underneath a fixed path. Both are generated by `@effected/schemastore-cli` from `lib/scripts/schemastore.config.ts`, which the scripts pass to the CLI explicitly (`src/` is action source only); it declares the two Effect Schema sources and derives every path and `$id` from `outputDir` + `baseUrl` + the label — nothing is spelled by hand. The base URL and label are `OUTPUT_SCHEMA_URL` / `OUTPUT_SCHEMA_VERSION` in `src/schema/silk-release-config.ts`; `SCHEMA_URL` and `INPUT_SCHEMA_URL` are template-literal derivations of the same two constants, and `__test__/schemastore-config.test.ts` pins them to the config-derived `$id`s. `pnpm schema:build` writes; `pnpm schema:check` is the same walk with no writes and fails when a build would write (stale artifact) or refuse (gate failure, or drift at a `published: true` label). The label is `published: false` today — the schema has never shipped — so a contract change regenerates `5.2` in place. Once a label is published, drift is answered by bumping `OUTPUT_SCHEMA_VERSION` and keeping the old label in the entries' `versions` as a frozen file. `--force` (sugar for `--drift=allow`) rewrites a published document in place — only to repair unparseable text. Runbook: `okf/runbooks/bump-output-schema-version.md`.
