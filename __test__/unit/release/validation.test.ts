...
import { runValidation } from "../../../src/release/validation.js";
...
describe("runValidation", () => {
	describe("released package (version diff succeeds)", () => {
		it.effect("reports publishOk: true and package present in packages when dryRun passes", () =>
			Effect.gen(function* () {
				// Arrange
				...
				// Act
				const report = yield* runValidation({ packageManager: "pnpm", targetBranch: "main", dryRun: false }).pipe(
		...
		it.effect("detects a single-root-workspace release (relativePath: '.') as a publishable package", () =>
			Effect.gen(function* () {
	...
	describe("dry-run failure", () => {
		it.effect("reports publishOk: false and npmReady: false when dryRun fails for an npm target", () =>
			Effect.gen(function* () {
				// Arrange
		...
		it.effect("reports githubPackagesReady: false when a GitHub Packages dry-run fails", () =>
			Effect.gen(function* () {
				// Arrange
	...
	describe("package not released (same version as target branch)", () => {
		it.effect("excludes the package from validation when current version equals base version", () =>
			Effect.gen(function* () {
	...
	describe("brand-new package (not on target branch)", () => {
		it.effect("includes a brand-new package when git show fails (package doesn't exist on target branch)", () =>
			Effect.gen(function* () {
	...
	describe("version-only package (no publish targets)", () => {
		it.effect("reports hasVersionOnlyPackages: true when a released package has no publish targets", () =>
			Effect.gen(function* () {
	...
	describe("no packages released (all versions unchanged)", () => {
		it.effect("returns publishOk: true and empty packages when no version changes are detected", () =>
			Effect.gen(function* () {
		...
		it.effect(
			"emits a warning-severity finding scoped to Publish Validation when zero packages have version diffs",
			() =>
				Effect.gen(function* () {
	...
	describe("built manifest decoding (SBOM components + degradation)", () => {
		...
		const runWithBuildDir = (buildDir: string) => {
			...
			return runValidation({ packageManager: "pnpm", targetBranch: "main", dryRun: false }).pipe(
				Effect.provide(layers),
			);
		};

		it.effect("turns the built manifest's dependencies into BOM components", () =>
			Effect.gen(function* () {
				...
				const report = yield* runWithBuildDir(dir);
		...
		it.effect("records a warning and degrades sbomOk when the built manifest will not decode", () =>
			Effect.gen(function* () {
		...
		it.effect("treats an ABSENT manifest as benign, not as degradation", () =>
			Effect.gen(function* () {
				...
				const report = yield* runWithBuildDir(withBuildDir(null));
	...
	describe("changeset counting (target-branch .changeset directory)", () => {
		it.effect("runs to completion when the target branch carries changeset files", () =>
			Effect.gen(function* () {
		...
		it.effect("runs to completion when git ls-tree for changesets fails (best-effort)", () =>
			Effect.gen(function* () {
	...
	describe("SBOM generation", () => {
		it.effect("generates SBOM for packages with provenance-enabled targets", () =>
			Effect.gen(function* () {
		...
		it.effect("generates SBOM for every published target regardless of provenance flag", () =>
			Effect.gen(function* () {
	...
	describe("validation findings", () => {
		it.effect("produces an error finding when a publish dry-run fails", () =>
			Effect.gen(function* () {
				// Arrange
		...
		it.effect("produces a warning finding when the generated SBOM is NTIA-incomplete", () =>
			Effect.gen(function* () {
		...
		it.effect("yields findings: [] for an all-pass run with an NTIA-complete SBOM", () =>
			Effect.gen(function* () {
	...
	describe("changeset-ignored packages", () => {
		it.effect("detectReleasedPackages drops changeset-ignored packages", () =>
			Effect.gen(function* () {
	...
	describe("directory-keyed builds", () => {
		it.effect("groups two targets sharing a directory into one build with one SBOM", () =>
			Effect.gen(function* () {
		...
		it.effect("produces two builds when a package's targets span two directories", () =>
			Effect.gen(function* () {
		...
		it.effect("carries the package-relative build directory (not an absolute path) into the report", () =>
			Effect.gen(function* () {
		...
		it("passes sbom-config supplier to Sbom.generate so the real BOM carries it and NTIA passes", async () => {
			...
			// Left on plain `it()`: the env var is restored through a promise
			// `.finally`, which has no faithful `Effect.gen` equivalent without
			// changing the cleanup semantics this test relies on.
		...
		it.effect("does not warn about a component-less BOM for a dependency-free package", () =>
			Effect.gen(function* () {
	...
	describe("topological ordering", () => {
		it.effect("validates released packages in dependency order, not workspace glob order", () =>
			Effect.gen(function* () {
