export const __rspack_esm_id = 257;
export const __rspack_esm_ids = [257];
...
17408(__unused_rspack_module, __webpack_exports__, __webpack_require__) {

// EXPORTS
__webpack_require__.d(__webpack_exports__, {
  createGitHubReleases: () => (/* binding */ createGitHubReleases)
});
...
// EXTERNAL MODULE: ./src/utils/registry-utils.ts
var registry_utils = __webpack_require__(36679);
;// CONCATENATED MODULE: ./src/utils/generate-publish-summary.ts
 ...
 */ function getPackagePageUrl(registry, packageName, version) {
    ...
    if ((0,registry_utils/* .isGitHubPackagesRegistry */.yQ)(registry)) {
        ...
        const { owner } = _actions_compat/* .context.repo */._O.repo;
...
;// CONCATENATED MODULE: ./src/utils/create-github-releases.ts
 ...
 */ function extractReleaseNotes(changelogPath, version) {
    if (!(0,external_node_fs_.existsSync)(changelogPath)) {
        return undefined;
    }
    const content = (0,external_node_fs_.readFileSync)(changelogPath, "utf-8");
 ...
 */ function findApiDocFile(directory, packageName) {
    ...
    const apiFilePath = (0,external_node_path_.join)(directory, apiFileName);
    if ((0,external_node_fs_.existsSync)(apiFilePath)) {
        (0,_actions_compat/* .debug */.Yz)(`Found API doc file: ${apiFilePath}`);
        return apiFilePath;
    }
    return undefined;
}
 ...
 */ async function configureGitIdentity() {
    const appSlug = (0,_actions_compat/* .getState */.Gu)("appSlug");
    ...
    (0,_actions_compat/* .debug */.Yz)(`Configuring git identity: ${userName} <${userEmail}>`);
    await (0,_actions_compat/* .exec */.m0)("git", [
        "config",
        "user.name",
        userName
    ]);
 ...
 */ async function createGitTag(tagName, message, dryRun) {
    if (dryRun) {
        (0,_actions_compat/* .info */.pq)(`[DRY RUN] Would create tag: ${tagName}`);
        return true;
    }
    const token = (0,_actions_compat/* .getState */.Gu)("token");
    if (!token) {
        (0,_actions_compat/* .error */.z3)("No token available for creating signed tag");
        return false;
    }
    const octokit = (0,_actions_compat/* .getOctokit */.QB)(token);
    const { owner, repo } = _actions_compat/* .context.repo */._O.repo;
    try {
            ...
            if (status === 422 && /already exists/i.test(refMessage)) {
                try {
                    ...
                    if (existingCommitSha === headSha) {
                        (0,_actions_compat/* .info */.pq)(`Tag ${tagName} already exists at ${headSha} — treating as idempotent success`);
                        return true;
                    }
                    (0,_actions_compat/* .warning */.$e)(`Tag ${tagName} already exists but points to ${existingCommitSha}, not ${headSha} — skipping (manual review needed)`);
                    return false;
                } catch (lookupErr) {
                    (0,_actions_compat/* .warning */.$e)(`Tag ${tagName} reportedly already exists but lookup failed: ${lookupErr instanceof Error ? lookupErr.message : String(lookupErr)}`);
                    return false;
                }
            }
            throw refErr;
        }
        (0,_actions_compat/* .info */.pq)(`Created and pushed tag: ${tagName}`);
        return true;
    } catch (err) {
        (0,_actions_compat/* .error */.z3)(`Failed to create tag ${tagName}: ${err instanceof Error ? err.message : String(err)}`);
        return false;
    }
}
/**
 ...
 */ async function uploadAssetIdempotent(octokit, owner, repo, releaseId, fileName, fileContent, existingAssets) {
    ...
    if (cached) {
        (0,_actions_compat/* .info */.pq)(`Asset ${fileName} already attached to release — skipping upload`);
        return cached;
    }
    try {
        ...
        if (status === 422 && /already_exists/i.test(err instanceof Error ? err.message : String(err))) {
            ...
            if (found) {
                ...
                existingAssets.set(fileName, result);
                (0,_actions_compat/* .info */.pq)(`Asset ${fileName} already attached (raced) — using existing copy`);
                return result;
            }
        }
        throw err;
 ...
 */ async function createGitHubReleases(tags, publishResults, _packageManager, dryRun) {
    ...
    const createdTags = [];
    const errors = [];
    (0,_actions_compat/* .startGroup */.Oh)("Creating GitHub releases");
    // Configure git identity for creating annotated tags (required for non-dry-run)
    if (!dryRun) {
        await configureGitIdentity();
    }
    for (const tag of tags){
        (0,_actions_compat/* .info */.pq)(`Processing release for ${tag.name}...`);
        ...
        if (associatedPackages.length === 0) {
            (0,_actions_compat/* .warning */.$e)(`No packages found for tag ${tag.name}`);
            continue;
        }
        // Build release notes
        let releaseNotes = "";
        for (const pkg of associatedPackages){
            ...
            const pkgPath = (0,find_package_path/* .findPackagePath */.di)(pkg.name);
            const changelogPaths = [];
            if (pkgPath) {
                changelogPaths.push((0,external_node_path_.join)(pkgPath, "CHANGELOG.md"));
            }
            // Fall back to root changelog for single-package repos
            changelogPaths.push((0,external_node_path_.join)(process.cwd(), "CHANGELOG.md"));
            let notes;
            for (const changelogPath of changelogPaths){
                (0,_actions_compat/* .debug */.Yz)(`Looking for CHANGELOG at: ${changelogPath}`);
                notes = extractReleaseNotes(changelogPath, pkg.version);
                if (notes) {
                    (0,_actions_compat/* .debug */.Yz)(`Found release notes for ${pkg.name}@${pkg.version} in ${changelogPath}`);
                    break;
                }
            }
        ...
        for (const pkg of associatedPackages){
            for (const target of pkg.targets.filter((t)=>t.success)){
                const registryName = (0,registry_utils/* .getRegistryDisplayName */.DN)(target.target.registry);
        ...
        if (dryRun) {
            (0,_actions_compat/* .info */.pq)(`[DRY RUN] Would create GitHub release for ${tag.name}`);
            releases.push({
                tag: tag.name,
                url: `https://github.com/${_actions_compat/* .context.repo.owner */._O.repo.owner}/${_actions_compat/* .context.repo.repo */._O.repo.repo}/releases/tag/${tag.name}`,
                id: 0,
                assets: []
            });
            continue;
        ...
        try {
            ...
            try {
                const created = await octokit.rest.repos.createRelease({
                    owner: _actions_compat/* .context.repo.owner */._O.repo.owner,
                    repo: _actions_compat/* .context.repo.repo */._O.repo.repo,
                    tag_name: tag.name,
                    name: tag.name,
                ...
                (0,_actions_compat/* .info */.pq)(`Created release: ${release.htmlUrl}`);
            } catch (createErr) {
                ...
                if (status === 422 && /already_exists|already exists/i.test(createMessage)) {
                    ...
                    (0,_actions_compat/* .info */.pq)(`Release for ${tag.name} already exists — reusing: ${release.htmlUrl}`);
                } else {
                    throw createErr;
                }
            }
            ...
            try {
                ...
                (0,_actions_compat/* .warning */.$e)(`Failed to enumerate existing assets for ${tag.name}: ${listErr instanceof Error ? listErr.message : String(listErr)}`);
            }
            const releaseInfo = {
                tag: tag.name,
                url: release.htmlUrl,
            ...
            for (const pkg of associatedPackages){
                ...
                if (targetsWithTarballs.length === 0) {
                    ...
                    (0,_actions_compat/* .warning */.$e)(`No tarball path recorded for ${pkg.name}@${pkg.version} — skipping release-asset upload`);
                    continue;
                }
                ...
                for (const targetResult of targetsWithTarballs){
                    ...
                    try {
                        const originalFileName = (0,external_node_path_.basename)(artifactPath);
                        ...
                        const fileContent = (0,external_node_fs_.readFileSync)(artifactPath);
                        (0,_actions_compat/* .info */.pq)(`Uploading asset: ${fileName}${needsPrefix ? ` (from ${targetResult.target.directory})` : ""}`);
                        const asset = await uploadAssetIdempotent(octokit, _actions_compat/* .context.repo.owner */._O.repo.owner, _actions_compat/* .context.repo.repo */._O.repo.repo, release.id, fileName, fileContent, existingAssetsByName);
                        (0,_actions_compat/* .info */.pq)(`Uploaded: ${asset.browserDownloadUrl}`);
                        const attestationResult = await (0,create_attestation/* .createReleaseAssetAttestation */.au)(artifactPath, pkg.name, pkg.version, dryRun);
                        ...
                        if (attestationResult.success && attestationResult.attestationUrl) {
                            (0,_actions_compat/* .info */.pq)(`  ✓ Created attestation: ${attestationResult.attestationUrl}`);
                        }
                        // Upload SBOM if available
                        if (targetResult.sbomPath && (0,external_node_fs_.existsSync)(targetResult.sbomPath)) {
                            try {
                                const sbomFileName = (0,external_node_path_.basename)(targetResult.sbomPath);
                                const sbomContent = (0,external_node_fs_.readFileSync)(targetResult.sbomPath);
                                (0,_actions_compat/* .info */.pq)(`Uploading SBOM: ${sbomFileName}`);
                                const sbomAsset = await uploadAssetIdempotent(octokit, _actions_compat/* .context.repo.owner */._O.repo.owner, _actions_compat/* .context.repo.repo */._O.repo.repo, release.id, sbomFileName, sbomContent, existingAssetsByName);
                                (0,_actions_compat/* .info */.pq)(`Uploaded SBOM: ${sbomAsset.browserDownloadUrl}`);
                                ...
                                (0,_actions_compat/* .warning */.$e)(`Failed to upload SBOM ${targetResult.sbomPath}: ${sbomErr instanceof Error ? sbomErr.message : String(sbomErr)}`);
                            }
                        }
                        ...
                        if (apiDocPath) {
                            try {
                                const apiDocFileName = (0,external_node_path_.basename)(apiDocPath);
                                ...
                                const apiDocContent = (0,external_node_fs_.readFileSync)(apiDocPath);
                                (0,_actions_compat/* .info */.pq)(`Uploading API doc: ${finalApiDocFileName}`);
                                const apiDocAsset = await uploadAssetIdempotent(octokit, _actions_compat/* .context.repo.owner */._O.repo.owner, _actions_compat/* .context.repo.repo */._O.repo.repo, release.id, finalApiDocFileName, apiDocContent, existingAssetsByName);
                                (0,_actions_compat/* .info */.pq)(`Uploaded API doc: ${apiDocAsset.browserDownloadUrl}`);
                                ...
                                (0,_actions_compat/* .warning */.$e)(`Failed to upload API doc ${apiDocPath}: ${apiDocErr instanceof Error ? apiDocErr.message : String(apiDocErr)}`);
                            }
                        }
                    } catch (err) {
                        (0,_actions_compat/* .warning */.$e)(`Failed to upload artifact ${artifactPath}: ${err instanceof Error ? err.message : String(err)}`);
                    }
                }
            ...
            if (releaseInfo.assets.length > 0) {
                try {
                    ...
                    (0,_actions_compat/* .info */.pq)(`Updated release notes with asset links`);
                } catch (err) {
                    (0,_actions_compat/* .warning */.$e)(`Failed to update release notes: ${err instanceof Error ? err.message : String(err)}`);
                }
            }
            releases.push(releaseInfo);
        } catch (err) {
            const errorMessage = `Failed to create release for ${tag.name}: ${err instanceof Error ? err.message : String(err)}`;
            (0,_actions_compat/* .error */.z3)(errorMessage);
