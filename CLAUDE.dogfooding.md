...
Every `@effected/*` and `@savvy-web/*` runtime dependency is declared as `catalog:effected` / `catalog:silk` in `package.json` — **the ranges are not written here and must not be hand-edited.** The catalogs are generated by the `@effected/pnpm-plugin-effect` and `@savvy-web/pnpm-plugin-silk` config dependencies pinned in `pnpm-workspace.yaml`, so bumping the whole line is bumping the plugin: `pnpm add --config @effected/pnpm-plugin-effect@<version>`, then `pnpm clean --lockfile && pnpm install`. As of 2026-09-08 (plugin `0.7.1`, `@savvy-web/pnpm-plugin-silk` `0.34.1`) that installs `@effected/npm 0.13.0`, `@effected/github-actions 0.11.0`, `@effected/github 0.9.0`, `@effected/github-references 0.2.0`, `@effected/git 0.12.0`, `@effected/package-json 0.14.0`, `@effected/workspaces 0.20.1`, `@effected/commands 0.6.1`, `@effected/sbom 0.5.0`, `@effected/memfs 0.6.0`, `@effected/markdown 0.9.1`, `@effected/semver 0.6.0`, `@effected/yaml 0.14.0`, `@effected/jsonc 0.9.0`, `@effected/schemastore 0.6.0` and `@savvy-web/silk-effects 7.5.1`; `effect`, `@effect/platform-node` and `@effect/vitest` come from `catalog:effect` at **4.0.0-rc.112**. Treat this list as dated evidence — read `node_modules` when it matters.
